...
export function listMarketplaceListings(search?: string) {
  const query = search ? `?search=${encodeURIComponent(search)}` : "";
  return requestJson<MarketplaceListing[]>(`/api/marketplace/listings${query}`);
}

export function listMarketplacePublications() {
  return requestJson<MarketplaceListing[]>("/api/marketplace/me/publications");
}

export function listMarketplaceSubscriptions() {
  return requestJson<MarketplaceSubscription[]>("/api/marketplace/me/subscriptions");
}

export function createMarketplaceListing(input: { source_strategy_id: string; title: string; summary: string; visibility?: "public" | "unlisted" }) {
  return requestJson<MarketplaceListing>("/api/marketplace/listings", { method: "POST", body: JSON.stringify(input) });
}

export function publishMarketplaceListing(listingId: string) {
  return requestJson<MarketplaceListing>(`/api/marketplace/listings/${encodeURIComponent(listingId)}/publish`, { method: "POST", body: "{}" });
}

export function archiveMarketplaceListing(listingId: string) {
  return requestJson<MarketplaceListing>(`/api/marketplace/listings/${encodeURIComponent(listingId)}/archive`, { method: "POST", body: "{}" });
}

export function subscribeMarketplaceListing(listingId: string, versionId?: string) {
  const idempotency_key = `web:${listingId}:${versionId ?? "current"}:${crypto.randomUUID()}`;
  const suffix = versionId ? "upgrade" : "subscribe";
  return requestJson<MarketplaceSubscription>(`/api/marketplace/listings/${encodeURIComponent(listingId)}/${suffix}`, {
    method: "POST",
    body: JSON.stringify(versionId ? { idempotency_key, version_id: versionId } : { idempotency_key })
  });
}

export function unsubscribeMarketplaceListing(listingId: string) {
  return requestJson<MarketplaceSubscription>(`/api/marketplace/listings/${encodeURIComponent(listingId)}/unsubscribe`, {
    method: "POST",
    body: JSON.stringify({ idempotency_key: `web:${listingId}:unsubscribe:${crypto.randomUUID()}` })
  });
}
...
export function getAdminSystemHealth() {
  return requestJson<AdminSystemHealth>("/api/admin/system-health");
}

export function listAdminAuditLog(params?: { limit?: number; offset?: number; organization_id?: string; action?: string }) {
  const search = new URLSearchParams();
  Object.entries(params ?? {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && String(value).trim()) search.set(key, String(value));
  });
  const suffix = search.toString() ? `?${search.toString()}` : "";
  return requestJson<AdminAuditEntry[]>(`/api/admin/audit-log${suffix}`);
}

export function getAdminQuotas() {
  return requestJson<AdminQuotasResponse>("/api/admin/quotas");
}

export function updateAdminQuotas(organizationId: string, limits: Record<string, number>) {
  return requestJson<{ organization_id: string; quotas: Record<string, number>; status: string }>(`/api/admin/quotas/${encodeURIComponent(organizationId)}`, {
    method: "PATCH",
...
export function approveStrategySpec(spec: Record<string, unknown>, approvalText: string, provenanceToken?: string | null) {
  return requestJson<StrategyBuilderApprovalResponse>("/api/strategies/builder/approve", {
    method: "POST",
    body: JSON.stringify({ spec, approved: true, approval_text: approvalText, provenance_token: provenanceToken ?? null })
  });
}
...
export function getOhlc(params: { symbol: string; start: string; end: string; interval?: string }) {
  const search = new URLSearchParams();
  search.set("symbol", params.symbol);
  search.set("start", params.start);
  search.set("end", params.end);
  if (params.interval) search.set("interval", params.interval);
  return requestJson<OhlcPayload>(`/api/backtests/ohlc?${search.toString()}`);
}
