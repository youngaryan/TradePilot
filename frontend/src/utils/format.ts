...
export function statusTone(status: string | null | undefined): "good" | "bad" | "warn" | "neutral" {
  ...
  if (status === "queued" || status === "running" || status === "dispatch_pending") return "warn";
  return "neutral";
}

export function jobDisplayStatus(job: { status?: string | null; stage?: string | null } | null | undefined): string {
  if (!job) return "idle";
  if (job.status === "queued" && job.stage === "dispatch_pending") return "dispatch_pending";
