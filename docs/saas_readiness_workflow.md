...
The app has a local-first SaaS shell and a production deployment topology around the quant research engine. The production topology includes Postgres, S3-compatible object storage, Redis/RQ workers and controller reconciliation, Stripe webhooks, structured observability, and admin operations. Deployment still requires operator-owned credentials, infrastructure, security review, and legal review.
...
The backend verifies and processes Stripe webhooks idempotently to synchronize checkout, renewal, cancellation, failed-payment, and plan changes when Stripe is configured. Local demo mode does not emulate external webhook delivery.
...
The included topology supports hardened sessions/MFA, scoped tenant authorization, mounted secrets or explicit AWS Secrets Manager references, Stripe webhook entitlement synchronization, Postgres/object storage, external Redis/RQ workers, metrics/traces/logging, rate limits, and admin health/audit/quota operations.

Operators still own infrastructure provisioning, backups and restore drills, secret rotation, external identity-policy decisions, provider contracts, incident response, and security review. TradePilot remains paper-only: it has no live broker routing, and legal review is required before charging users or making investment-advice or hypothetical-performance claims.
