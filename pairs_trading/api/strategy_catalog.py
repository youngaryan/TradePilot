...
def build_strategy_catalog() -> list[dict[str, Any]]:
    ...
    items: list[StrategyCatalogItem] = [
        ...
        _directional_item(
            strategy_id="oxford_combined_donchian",
            name="Oxford Combined Donchian",
            difficulty="Advanced",
            summary="Oxford-inspired Donchian breakout with a separate trailing channel exit.",
            how_it_works="Uses prior close-price channels for breakout entry and a shorter opposite channel to return to neutral.",
            best_for="Diversified trend-following research where the entry and exit horizon should be independently configurable.",
            watch_out="Oxford's source uses OHLC stop orders; this project uses close-only channel approximations.",
            key_parameters=("breakout_window", "breakout_exit_window", "strategy_cost_bps"),
            cli_args="--breakout-window 100 --breakout-exit-window 50",
            params={"breakout_window": 100, "breakout_exit_window": 50},
        ),
        _directional_item(
            strategy_id="oxford_price_momentum",
            name="Oxford Price Momentum",
            difficulty="Intermediate",
            summary="Fast and slow price momentum model based on Oxford's Kaufman benchmark review.",
            how_it_works="Requires both fast and slow price differences to agree, then exits when fast momentum flips.",
            best_for="Simple, low-parameter trend continuation research on liquid cross-asset instruments.",
            watch_out="It is intentionally simple and can lag sharp reversals.",
            key_parameters=("momentum_lookbacks", "strategy_cost_bps"),
            cli_args="--momentum-lookbacks 90 120",
            params={"momentum_lookbacks": [90, 120]},
        ),
        _directional_item(
            strategy_id="oxford_dual_momentum_roc",
            name="Oxford Dual Momentum ROC",
            difficulty="Intermediate",
            summary="Dual momentum strategy using slow ROC as filter and fast ROC as setup.",
            how_it_works="Trades only when fast and slow rate-of-change signs agree, with a configurable time exit.",
            best_for="Testing the site's longer-holding-period momentum observation without adding many filters.",
            watch_out="Time exits are blunt; validate holding periods out of sample.",
            key_parameters=("momentum_lookbacks", "breakout_exit_window", "strategy_cost_bps"),
            cli_args="--momentum-lookbacks 60 120 --breakout-exit-window 60",
            params={"momentum_lookbacks": [60, 120], "breakout_exit_window": 60},
        ),
        _directional_item(
            strategy_id="oxford_bollinger_momentum",
            name="Oxford Bollinger Momentum",
            difficulty="Intermediate",
            summary="Three-phase Bollinger trend model that enters outside the bands and exits at the middle band.",
            how_it_works="Uses a slower moving average and volatility envelope to classify long, short, or neutral regimes.",
            best_for="Trend research where a volatility-adjusted breakout threshold is preferable to a raw channel.",
            watch_out="Band breakouts can whipsaw when volatility expands without follow-through.",
            key_parameters=("bollinger_window", "bollinger_num_std", "strategy_cost_bps"),
            cli_args="--bollinger-window 80 --bollinger-num-std 2.0",
            params={"bollinger_window": 80, "bollinger_num_std": 2.0},
        ),
        _directional_item(
            strategy_id="oxford_keltner_three_phase",
            name="Oxford Keltner Three Phase",
            difficulty="Intermediate",
            summary="Keltner channel trend model with long, short, and neutral states.",
            how_it_works="Builds an EMA centerline with close-to-close ATR-style bands and exits when price crosses back through the center.",
            best_for="Breakout research that should adapt entry thresholds to recent volatility.",
            watch_out="The source uses OHLC true range; this implementation uses a close-only ATR proxy.",
            key_parameters=("keltner_window", "keltner_atr_multiplier", "strategy_cost_bps"),
            cli_args="--keltner-window 60 --keltner-atr-multiplier 1.5",
            params={"keltner_window": 60, "keltner_atr_multiplier": 1.5},
        ),
        _directional_item(
            strategy_id="oxford_normalized_regression_slope",
            name="Oxford Normalized Regression Slope",
            difficulty="Advanced",
            summary="Trend model that trades sufficiently positive or negative normalized regression slope.",
            how_it_works="Fits a rolling linear slope, normalizes it by price, enters beyond a growth threshold, and exits when slope crosses zero.",
            best_for="Smoother trend estimation than simple price momentum while keeping a direct market rationale.",
            watch_out="Regression windows introduce lag and the normalization is an approximation of the public article.",
            key_parameters=("trend_window", "strategy_cost_bps"),
            cli_args="--trend-window 120",
            params={"trend_window": 120},
        ),
        _directional_item(
            strategy_id="oxford_bollinger_percent_b_reversal",
            name="Oxford Bollinger Percent B Reversal",
            difficulty="Advanced",
            summary="Trend-filtered Bollinger percent-b pullback strategy.",
            how_it_works="Buys repeated low percent-b readings only in an uptrend and shorts repeated high readings only in a downtrend.",
            best_for="Mean-reversion sleeves that still respect the larger trend direction.",
            watch_out="A persistent trend break can make pullbacks continue instead of mean revert.",
            key_parameters=("bollinger_window", "bollinger_num_std", "trend_window", "strategy_cost_bps"),
            cli_args="--bollinger-window 20 --bollinger-num-std 2.0 --trend-window 200",
            params={"bollinger_window": 20, "bollinger_num_std": 2.0, "trend_window": 200},
        ),
        _directional_item(
            strategy_id="oxford_rsi2_pullback",
            name="Oxford RSI2 Pullback",
            difficulty="Intermediate",
            summary="Long-only 2-period RSI pullback model for equity-index style markets.",
            how_it_works="Requires price above a long moving average, buys very low RSI2 readings, and exits on a short moving-average recovery.",
            best_for="Testing short-term equity pullback behavior as a diversifier to trend-following systems.",
            watch_out="Oxford's own summary says it underperformed alternative momentum models; keep size conservative.",
            key_parameters=("trend_window", "lower_entry", "breakout_exit_window", "strategy_cost_bps"),
            cli_args="--trend-window 200 --lower-entry 10 --breakout-exit-window 10",
            params={"trend_window": 200, "lower_entry": 10, "breakout_exit_window": 10},
        ),
        _directional_item(
            strategy_id="oxford_wyckoff_range_reversion",
            name="Oxford Wyckoff Range Reversion",
            difficulty="Advanced",
            summary="Close-only approximation of Wyckoff false-breakout mean reversion.",
            how_it_works="Looks for closes outside a rolling range followed by re-entry, then targets the range midpoint or a time exit.",
            best_for="Researching bear-trap and bull-trap behavior without requiring proprietary pattern annotations.",
            watch_out="The public source uses richer swing/range geometry than a close-only rolling range.",
            key_parameters=("sma_window", "breakout_exit_window", "strategy_cost_bps"),
            cli_args="--sma-window 40 --breakout-exit-window 10",
            params={"sma_window": 40, "breakout_exit_window": 10},
        ),
        _directional_item(
            strategy_id="oxford_volatility_clustering",
            name="Oxford Volatility Clustering",
            difficulty="Advanced",
            summary="DELTA20-adjacent public continuation model after unusually large moves.",
            how_it_works="Identifies the largest recent close-to-close move versus its history, trades in the same direction, and exits after a short hold.",
            best_for="Testing the Mandelbrot-style volatility clustering concept discussed across the Oxford data posts.",
            watch_out="Oxford's OHLC wide-range definitions and ORB variants are approximated with close-only movement.",
            key_parameters=("volatility_window", "strategy_cost_bps"),
            cli_args="--volatility-window 20",
