...
def fetch_ohlc_frame(symbol: str, start: str, end: str, interval: str = "1d") -> pd.DataFrame:
    """
    Fetch a single symbol's OHLCV bars from Yahoo Finance.

    Unlike the close-price matrix used by the backtest engine, this keeps the
    full open/high/low/close/volume columns so the UI can render candlesticks.
    Derived intervals (4h) are aggregated from their base interval with proper
    OHLC semantics (first/max/min/last) rather than close-only resampling.
    """
    request = DataRequest.from_inputs(symbols=[symbol], start=start, end=end, interval=interval)
    provider_interval = base_interval_for(request.interval)
    raw = yf.download(
        [request.symbols[0]],
        start=request.start,
    ...
    ticker = request.symbols[0]
    if isinstance(raw.columns, pd.MultiIndex):
        def _col(name: str) -> pd.Series:
            if name in raw.columns.get_level_values(0):
                block = raw[name]
                return block[ticker] if ticker in block.columns else block.iloc[:, 0]
        ...
        frame = raw.rename(columns={c: str(c).lower() for c in raw.columns})
    ...
    if provider_interval != request.interval:
        frame = frame.resample(request.interval).agg(_OHLC_AGG).dropna(subset=["close"])
    return frame
