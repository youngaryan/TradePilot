...
class CommitteeSignalFollowerPipeline(WalkForwardStrategy):
    ...
    def __init__(
        ...
        *,
        ...
        self.decision_horizons = tuple(str(item).lower() for item in decision_horizons) if decision_horizons else None
        self.name = name
    ...
    def run_fold(self, train_data: pd.DataFrame, test_data: pd.DataFrame) -> StrategyOutput:
        ...
        for symbol in available:
            ...
            for d in decisions:
                if self.decision_horizons and str(d.horizon or "").lower() not in self.decision_horizons:
                    continue
        ...
        return StrategyOutput(
            ...
            diagnostics={
                ...
                "decision_horizons": list(self.decision_horizons or []),
            },
        ).validate(extra_columns=("gross_return",))
