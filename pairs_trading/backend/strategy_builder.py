...
class StrategyBuilderService:
    ...
    def approve(self, *, organization_id: str, user_id: str, spec: dict[str, Any], approval_text: str) -> dict[str, Any]:
        ...
        record = self.store.create_user_strategy(
            ...
            approval={"approval_text": approval_text, "approved_at_utc": utc_now_iso(), "source": "strategy_builder"},
            status="active",
        ...
        self.store.record_audit_log(
            ...
            target_id=record["id"],
            metadata={"strategy_name": record["name"], "risk_level": risk_level, "version": record["version"], "spec": validation.spec},
        )
    ...
    def catalog_item(record: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "id": record["id"],
            "name": f"{record['name']} (AI)",
            "family": "User-created",
            "difficulty": f"AI generated / {record.get('risk_level', 'medium')} risk",
            ...
            "watch_out": "AI-generated strategies are user-approved research hypotheses, not guaranteed profitable trading systems.",
