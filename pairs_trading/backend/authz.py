...
def require_auth_context(settings: BackendSettings, *, machine_scope: str | None = None) -> Callable[..., RequestContext]:
    ...
    def dependency(
        ...
        try:
            ...
            if context.user.get("machine"):
                if machine_scope is None:
                    raise HTTPException(
                        status_code=403,
                        detail={"code": "browser_session_required", "message": "This endpoint requires a browser session."},
                    )
                scopes = {str(scope).lower() for scope in context.user.get("scopes", [])}
                if "*" not in scopes and machine_scope.lower() not in scopes:
                    raise HTTPException(
                        status_code=403,
                        detail={"code": "api_key_scope_required", "scope": machine_scope, "message": f"This machine API key is missing the required scope: {machine_scope}."},
                    )
            return context
...
def require_csrf(settings: BackendSettings) -> Callable[..., None]:
    ...
    def dependency(
        ...
        if credentials is not None:
            if not credentials.credentials.startswith("qops_"):
                raise HTTPException(
                    status_code=401,
...
def require_paid_context(settings: BackendSettings, *, feature: str = "premium feature", machine_scope: str | None = None) -> Callable[..., RequestContext]:
    auth_dependency = require_auth_context(settings, machine_scope=machine_scope)
