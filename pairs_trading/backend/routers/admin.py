...
from ..schemas import AdminQuotaUpdateRequest, AdminStrategyStatusUpdateRequest, AdminUserUpdateRequest
...
def build_admin_router(settings: BackendSettings) -> APIRouter:
    ...
    def update_quotas(
        ...
        if not limits:
            raise HTTPException(status_code=422, detail="Provide at least one recognized quota limit.")
        return admin_service.update_quotas(organization_id=organization_id, payload=limits, actor_user_id=str(ctx.user["id"]))
