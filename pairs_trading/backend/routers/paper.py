...
def build_paper_router(settings: BackendSettings) -> APIRouter:
    ...
    job_read_context = require_auth_context(settings, machine_scope="paper:read")
    ...
    def get_summary(
        ...
        deployment_id: str | None = Query(default=None, description="Optional tenant-owned paper deployment id."),
        legacy_compat: bool = Query(default=False, description="Allow legacy local filesystem state in development."),
    ...
    def list_strategies(
        ...
        strategies = service.list_strategies(organization_id=ctx.organization_id, paper_agent_id=paper_agent_id, deployment_id=deployment_id, allow_legacy=legacy_compat)
    ...
    def get_strategy(
        ...
        strategy = service.get_strategy(organization_id=ctx.organization_id, strategy_name=strategy_name, paper_agent_id=paper_agent_id, deployment_id=deployment_id, allow_legacy=legacy_compat)
        if strategy is None:
