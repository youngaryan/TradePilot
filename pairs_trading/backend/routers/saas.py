...
from ..saas import AuthRateLimitError, AuthService, BillingInputError, BillingProviderError, BillingService, BillingWebhookProcessingError, CSRF_COOKIE_NAME, MFA_COOKIE_NAME, RequestContext, SaaSService, SESSION_COOKIE_NAME
from ..schemas import (
    ApiKeyCreateRequest,
...
def build_saas_router(settings: BackendSettings) -> APIRouter:
    ...
    def context(
        ...
        if credentials is not None and not credentials.credentials.startswith("qops_"):
            raise HTTPException(
                status_code=401,
                detail={"code": "machine_api_key_required", "message": "Bearer auth is reserved for scoped machine API keys."},
            )
        token = credentials.credentials if credentials is not None else request.cookies.get(SESSION_COOKIE_NAME)
        if not token:
            raise HTTPException(status_code=401, detail="Login required.")
        try:
            ctx = auth_service.authenticate(token=token, organization_id=active_organization_id)
            if ctx.user.get("machine"):
                raise HTTPException(
                    status_code=403,
                    detail={"code": "browser_session_required", "message": "This endpoint requires a browser session."},
                )
            return ctx
    ...
    def login(request_body: LoginRequest, response: Response, request: Request) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=429, detail={"code": "auth_rate_limited", "message": str(exc)}) from exc
    ...
    def logout(
        ...
        request: Request,
        credentials: HTTPAuthorizationCredentials | None = Depends(bearer),
        _: RequestContext = Depends(context),
    ...
    def verify_email_request(request_body: EmailVerificationSendRequest, request: Request) -> dict[str, Any]:
        try:
            return auth_service.request_email_verification(
                email=request_body.email,
                client_id=request.client.host if request.client else None,
            )
        except AuthRateLimitError as exc:
    ...
    def password_reset_request(request_body: PasswordResetRequest, request: Request) -> dict[str, str]:
        try:
    ...
    def mfa_verify(
        ...
        try:
            payload = auth_service.verify_mfa_code(
        ...
        if not token:
            raise HTTPException(status_code=401, detail="Login required.")
        if payload.get("sessions_revoked"):
            response.delete_cookie(SESSION_COOKIE_NAME, path="/", domain=settings.cookie_domain)
            response.delete_cookie(CSRF_COOKIE_NAME, path="/", domain=settings.cookie_domain)
            response.delete_cookie(MFA_COOKIE_NAME, path="/", domain=settings.cookie_domain)
            return payload
    ...
    def mfa_setup(request_body: MfaSetupRequest, request: Request, ctx: RequestContext = Depends(context), __: None = Depends(csrf_guard)) -> dict[str, Any]:
        try:
            return auth_service.setup_mfa(
                user_id=str(ctx.user["id"]),
                password=request_body.password,
                rotate=request_body.rotate,
                current_code=request_body.current_code,
                client_id=request.client.host if request.client else None,
            )
        except PermissionError as exc:
            raise HTTPException(status_code=403, detail=str(exc)) from exc
    ...
    def checkout(request: BillingCheckoutRequest, ctx: RequestContext = Depends(context), _: None = Depends(csrf_guard)) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=400, detail={"code": "billing_request_invalid", "message": "The billing request is invalid."}) from None
        except BillingProviderError:
            raise HTTPException(status_code=502, detail={"code": "billing_provider_unavailable", "message": "Billing is temporarily unavailable."}) from None
    ...
    def portal(request: BillingPortalRequest, ctx: RequestContext = Depends(context), _: None = Depends(csrf_guard)) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=400, detail={"code": "billing_return_url_invalid", "message": "The billing return URL is not allowed."}) from None
        except BillingProviderError:
            raise HTTPException(status_code=502, detail={"code": "billing_provider_unavailable", "message": "Billing is temporarily unavailable."}) from None
    ...
    def sync_billing(ctx: RequestContext = Depends(context), _: None = Depends(csrf_guard)) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=409, detail={"code": "billing_subscription_invalid", "message": "The billing subscription could not be synchronized."}) from None
        except BillingProviderError:
            raise HTTPException(status_code=502, detail={"code": "billing_provider_unavailable", "message": "Billing is temporarily unavailable."}) from None
    ...
    async def stripe_webhook(request: Request) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=400, detail={"code": "billing_signature_invalid", "message": "The billing webhook signature is invalid."}) from None
        except BillingInputError:
            raise HTTPException(status_code=400, detail={"code": "billing_webhook_invalid", "message": "The billing webhook payload is invalid."}) from None
        except BillingWebhookProcessingError:
            raise HTTPException(status_code=503, detail={"code": "billing_webhook_retry", "message": "Billing webhook processing should be retried."}) from None
        except BillingProviderError:
            raise HTTPException(status_code=503, detail={"code": "billing_webhook_unavailable", "message": "Billing webhook processing is unavailable."}) from None
