...
from ..core.timeframes import TradingMode
...
class PaperRunRequest(BaseModel):
    deployment_id: str | None = Field(
        default=None,
        description="Optional tenant-owned paper deployment id.",
    )
    project_id: str | None = Field(
        default=None,
        description="Optional tenant-owned project id for the deployment.",
    )
...
class BacktestRunRequest(BaseModel):
    ...
    trading_mode: TradingMode | None = Field(
        default=None,
        description="Optional trading mode. daily forces daily bars; short_term uses hourly execution with 4-hour signal context.",
    )
    compare_modes: bool = Field(
        default=False,
        description="When true, run daily and short-term simulations side by side for the same pipeline request.",
    )
...
class AdminQuotaUpdateRequest(BaseModel):
    ...
    news_pages: int | None = Field(default=None, ge=0, le=10_000_000)
    artifact_storage_mb: int | None = Field(default=None, ge=0, le=10_000_000)


class MarketplaceListingCreateRequest(BaseModel):
    source_strategy_id: str = Field(min_length=8, max_length=160)
    title: str = Field(min_length=3, max_length=120)
    summary: str = Field(min_length=12, max_length=1_000)
    visibility: str = Field(default="public", pattern="^(public|unlisted)$")


class MarketplaceListingUpdateRequest(BaseModel):
    title: str | None = Field(default=None, min_length=3, max_length=120)
    summary: str | None = Field(default=None, min_length=12, max_length=1_000)
    visibility: str | None = Field(default=None, pattern="^(public|unlisted)$")


class MarketplaceVersionCreateRequest(BaseModel):
    source_strategy_id: str | None = Field(default=None, min_length=8, max_length=160)
...
class MarketplaceUpgradeRequest(MarketplaceMutationRequest):
    version_id: str = Field(min_length=8, max_length=160)


class MarketplaceModerationRequest(BaseModel):
    status: str = Field(pattern="^(published|suspended)$")
...
class MfaSetupRequest(BaseModel):
    ...
    rotate: bool = False
    current_code: str | None = Field(default=None, min_length=6, max_length=12)
...
class BillingCheckoutRequest(BaseModel):
    ...
    request_id: str | None = Field(
        ...
        description="Optional retry identifier used only to scope a server-derived Stripe idempotency key.",
    )
