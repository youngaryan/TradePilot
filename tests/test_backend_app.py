...
class BackendAppTests(unittest.TestCase):
    ...
    def test_backend_routes_return_paper_payload(self) -> None:
        ...
        summary = client.get("/api/paper/summary?legacy_compat=true", headers=headers)
        strategy = client.get("/api/paper/strategies/trend?legacy_compat=true", headers=headers)
        missing = client.get("/api/paper/strategies/missing?legacy_compat=true", headers=headers)
    ...
    def test_saas_routes_auth_workspace_billing_and_details(self) -> None:
        ...
        self.assertEqual(client.get("/api/auth/me", headers=machine_headers).status_code, 401)
    ...
    def test_admin_rbac_and_payment_wall_are_enforced_server_side(self) -> None:
        ...
        self.assertEqual(inactive_login.json()["detail"], "Invalid email or password.")
    ...
    def test_sentiment_job_routes_report_progress_completion_and_failures(self) -> None:
        ...
        self.assertIn("newsapi headline provider credentials are not configured", failed_payload["error"])
