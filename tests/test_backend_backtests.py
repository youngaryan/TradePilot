...
class BackendBacktestTests(unittest.TestCase):
    ...
    def test_paper_run_job_accepts_inline_multi_agent_replay(self) -> None:
        ...
        with patch(
            ...
            metadata = SQLiteMetadataStore(workspace / "metadata.sqlite3")
            deployment = metadata.get_paper_deployment(
                organization_id=job_payload["organization_id"],
                deployment_id=job_payload["result"]["deployment_id"],
            )
            self.assertEqual(deployment["config"]["strategies"][1]["name"], "stat_arb_agent")
